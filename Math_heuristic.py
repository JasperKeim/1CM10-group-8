...
####################################################################################
#Input_file
File_name = '6M140.csv'                         # '6M140.csv' or 'example_3.csv'
####################################################################################
...
def chromosome_generator():
    #Shuffling the job_list so that the order in which (V^I) is filled is different each time.
    Job_list = []
    for i in Jobs:
        Job_list.append(i)
    random.shuffle(Job_list)
    #The shuffled order is now used to create a chromosome (i.e. individual), an random machine is allocated to V^M.
    Chromosome = []
    for i in Job_list:
